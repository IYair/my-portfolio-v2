...
export default function ProjectCard({ project, featured = false }: ProjectCardProps) {
  return (
    <div className="group relative overflow-hidden rounded-xl border border-foreground/10 bg-background shadow-sm transition-all duration-300 hover:shadow-2xl hover:border-foreground/20">
      {/* Featured Badge */}
      {featured && (
        <div className="absolute right-4 top-4 z-10 flex items-center gap-1 rounded-full bg-gradient-to-r from-yellow-500 to-orange-500 px-3 py-1 text-xs font-semibold text-white shadow-lg">
        ...
        <div className="flex items-center gap-4 border-t border-foreground/10 pt-4">
          ...
          {project.githubUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group/link flex items-center gap-1 text-sm font-medium text-foreground/60 transition-all hover:gap-2 hover:text-foreground"
            >
